...
public static class BillStack_Patches {
    ...
    public static readonly RecipeCollector Collector = new();
    ...
    public static void ITab_Bills_Pre() 
        => currentFlag = Generators.WorkTable;

    [HarmonyPostfix]
    [HarmonyPatch(typeof(ITab_Bills), "FillTab")]
    public static void ITab_Bills_Post() 
    ...
    [HarmonyPatch(typeof(FloatMenuOption), MethodType.Constructor,
        typeof(string), typeof(Action), typeof(ThingDef), typeof(Texture2D), typeof(ThingStyleDef), 
        typeof(bool), typeof(MenuOptionPriority), typeof(Action<Rect>), typeof(Thing), typeof(float),
        typeof(Func<Rect, bool>), typeof(WorldObject), typeof(bool), typeof(int), typeof(int?))]
    public static void FloatMenuOption_Ctor1(FloatMenuOption __instance,
                                            ref bool ___drawPlaceHolderIcon,
                                            ref Texture2D ___itemIcon) 
        => OnNewFloatMenuOption(__instance, ref ___drawPlaceHolderIcon, ref ___itemIcon);

    [HarmonyPostfix]
    [HarmonyPatch(typeof(FloatMenuOption), MethodType.Constructor,
        typeof(string), typeof(Action), typeof(ThingDef), typeof(ThingStyleDef), 
        typeof(bool), typeof(MenuOptionPriority), typeof(Action<Rect>), typeof(Thing), typeof(float),
        typeof(Func<Rect, bool>), typeof(WorldObject), typeof(bool), typeof(int), typeof(int?))]
    public static void FloatMenuOption_Ctor2(FloatMenuOption __instance,
                                            ref bool ___drawPlaceHolderIcon,
                                            ref Texture2D ___itemIcon) 
        => OnNewFloatMenuOption(__instance, ref ___drawPlaceHolderIcon, ref ___itemIcon);

    private static void OnNewFloatMenuOption(FloatMenuOption option,
                                             ref bool drawPlaceHolder,
                                             ref Texture2D icon) {
        if (Active(Generators.WorkTable)) {
            Collector.Add(option);
        } else if (Active(Generators.Operations) && drawPlaceHolder) {
    ...
    public static void GenerateSurgeryOption(FloatMenuOption __result, RecipeDef recipe, Pawn pawn, BodyPartRecord part) {
        if (Active(Generators.Operations)) {
            Collector.Add(new BillMenuEntry(__result, recipe, pawn, part));
        }
    }

    public class RecipeCollector {
        public readonly List<BillMenuEntry> Entries = [];
